...
#'
#' @param Folder Name of the subdirectory of the working directory storing image files
#' @param File.Ext File extension of the image file. Either ".bmp",".jpg",".png", or "", the last one for without extensions.
#' @param BodyL.File Name of the image file for the lateral silhouette of the body.
#' @param BodyV.File Name of the image file for the ventral silhouette of the body.
#' @param Cfin.File Name of the image file for the planar silhouette of the caudal fin.
#' @param Dfin.File Name of the image file for the planar silhouette of the dorsal fin, if any.
#' @param D2fin.File Name of the image file for the planar silhouette of the second dorsal fin, if any.
#' @param Ffin.File Name of the image file for the planar silhouette of the forefin or pectoral flipper.
#' @param Hfin.File Name of the image file for the planar silhouette of the hindfin or pelvic flipper.
#' @param Afin.File Name of the image file for the planar silhouette of the anal fin, if any.
#' @param Ceph.File Name of the image file for the planar silhouette of the cephalofoil, if any.
#' @param n1 Superelliptical exponent used in the first model.2.0 is recommended for marine tetrapods.
#' @param n2 Superelliptical exponent used in the second model. 3.0 is recommended for marine tetrapods.
...
#' @param Interpolate Logical. Whether to smooth the outline coordinates with local regression.
#' @param nn.b Nearest neighbor component of locfit() used for body reconstrurciton.
#' @param nn.f Nearest neighbor component of locfit() used for fin/flipper reconstrurciton.
#' @param Use.Weight Logical. Whether to add weight to the tips when using locfit()
#' @param Add.Tip Logical. Whether to add tips to the coordinates. TRUE is recommended.
#' @param tip.radius Radius of the tip to be added. 0.0001 is recommended.
#' @param a.per.slice Number of angular division per slice.
...
#' @param Mesh.Col Color of the mesh.
...
#' @param n.fin.slice Number of segments along the span of the fin. Moot unless N.Fin.Slice = "provided".
...
#' @param Ffin Logical. If the forefin/pectoral flipper is present.
#' @param Hfin Logical. If the hindfin/pelvic flipper is present.
#' @param Afin Logical. If the anal fin is present.
#' @param Ceph Logical. If the cephalofoil fin is present.
...
#' @param cfin.rot Rotation angle of the caudal fin around Z axis (cranio-caudal axis). 0 for vertical, pi/2 for horizontal fins.
...
#' @examples rm(Plesiosaurus)
#' @examples paleomass(Folder="Plesiosaurus",fork.l=2.94,Cfin=F,Dfin=F,ffin.onset=450,
...
#' @examples rm(Sphyrna)
#' @examples paleomass(Folder="Sphyrna",fork.l=2,n2=1.8,Afin=T,D2fin=T,Ceph=T,
#' @examples     ffin.onset=300, ffin.adj.up=-120,ffin.adj.med=40,ffin.spread=pi/4,
#' @examples     ffin.sweep=-pi*0/180,
#' @examples     hfin.onset=550,hfin.adj.up=10,hfin.adj.med=40,hfin.spread=pi/4,hfin.thick=10,
#' @examples     hfin.sweep=pi*5/180,
...
paleomass <- function(
    ...
    Cfin = T,           # Whether caudal fin is present--T or F
    Dfin = T,           # Whether dorsal fin is present--T or F
    D2fin = F,          # Whether second dorsal fin is present--T or F
    Ffin = T,           # Whether forefin (pectoral fin) is present--T or F
    Hfin = T,           # Whether hinDfin (pelvic fin) is present--T or F
    Afin = F,           # Whether anal fin is present--T or F
    Ceph = F,           # Whether cephalofoil is present--T or F
    ...
    Test.Run = F)       # Whether to set all positions and rotations for test
{
  ### Variable settings
